...
function get_epicwin_box() { 
	...
	if($_POST['action'] == 'subscribe') {
		...
		if ($errors) {
			...
			$query = $wpdb->query($wpdb->prepare("SELECT email FROM epicwin_feed WHERE email='{$_POST['sub_email']}'"));
			if (count($query) > 0) {
				$wpdb->query($wpdb->prepare("UPDATE epicwin_feed SET opt_in=1 WHERE email='{$_POST['sub_email']}'"));
	...
	if (!current_user_can('manage_options')) {
		...
		<div class="wrap">
			...
			<table class="form-table" style="width: 980px;">
				<tbody>
					<tr valign="top">
						<td><?php _e('Export file options') ?>:</td>
						<td>
					...
					<tr>
						<td><?php _e('Import file options'); ?>: </td>
						<td>
					...
					<tr valign="top">
						<td><?php _e('Newsletter Subscribers') ?>:</td>
						<td>
					...
					<tr>
						<td colspan="2">
							<h3><?php _e('Please if you like this plugin and you use it, help keep the development going. <br />Your donation ensures the cuntinous development of the plugin. <br />Any donation amount is appreciated.') ?></h3>
...
function unsubscribe() {
	...
	if(isset($_GET['unsub_id'])):
		...
		if ($queryResult > 0): ?>
			<div id="unsubscribe">
				<p><?php _e('You have been removed form our mailing list. If you would like to join the mailing list in the future, just fill in the subscribe box on the sidebar') ?>.</p>
