...
function new_post_alert() {
	...
	if ($_POST['original_post_status'] !== 'publish' && $_POST['post_type'] == 'post') {
		...
		if (count($query) > 0) {
			...
			$from = array(get_userdata(1)->user_email => get_userdata(1)->first_name . ' ' . get_userdata(1)->last_name);
			...
			$body .= (get_option('epicwin_email_message')) ? get_option('epicwin_email_message') : 'New post from ' . get_bloginfo('name') . '. Click the link bellow to view it: <br><a href="' . $queryPosts[0]->guid . '">' . $queryPosts[0]->post_title . '</a><br><br>';
			...
			$body = preg_replace('/\{link\}/', '<a href="' . $queryPosts[0]->guid . '">' . $queryPosts[0]->post_title . '</a>', $body);
			
			require_once 'lib/swift_required.php';
			
			$message = Swift_Message::newInstance()->setSubject($subject)->setFrom($from)->setTo($to)->setBody($body)->setContentType("text/html");
